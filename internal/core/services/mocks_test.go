...
// This file contains in-memory fake implementations of all repository and
// email interfaces. They are used in tests instead of real Postgres/email.
//
// Pattern: each mock has a map (or slice) storing data, plus optional
// function fields (e.g. CreateFn) that a specific test can override to
// inject errors or custom behaviour.
//
// Why function fields? Because most tests just want the happy path, but
// some tests want "GetByEmail returns an error". Instead of building
// elaborate state, the test just sets:
//
//   repo.GetByEmailFn = func(...) { return nil, errors.New("db down") }
...
type mockPlanRepo struct {
	plans map[string]*domain.Plan // keyed by name
}
...
// mockEmailSender records what emails were sent so tests can assert on them.
