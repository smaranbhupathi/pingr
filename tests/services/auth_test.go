...
func newTestAuthService() (inbound.AuthService, *mockUserRepo, *mockEmailSender) {
	userRepo := newMockUserRepo()
	planRepo := newMockPlanRepo()
	channelRepo := newMockAlertChannelRepo()
	emailSender := &mockEmailSender{}

	svc := services.NewAuthService(userRepo, planRepo, channelRepo, emailSender, services.AuthServiceConfig{
...
// registerAndVerify is a test helper that registers a user and simulates
// clicking the email verification link. Returns the user's ID.
func registerAndVerify(t *testing.T, svc inbound.AuthService, userRepo *mockUserRepo, email, username, password string) uuid.UUID {
	t.Helper()
...
func TestForgotPassword_UnknownEmail_NoError(t *testing.T) {
	// Security: must NOT error on unknown emails — prevents email enumeration attacks.
	svc, _, _ := newTestAuthService()

	err := svc.ForgotPassword(context.Background(), "unknown@example.com")
	if err != nil {
		t.Errorf("ForgotPassword should return nil for unknown emails, got %v", err)
	}
}
...
func TestResetPassword_Success(t *testing.T) {
	...
	if err := svc.ResetPassword(ctx, user.ResetToken, "newpass123"); err != nil {
